...
logger.add("logs/info.log", format="{time} {level} {message}", level="INFO", rotation="50 MB", compression="zip", mode="w")
...
async def run_bot(message: types.Message):
    ...
    await config.StateMachine.start_question.set()  # Показываем следующий вопрос
...
@dp.message_handler(state=config.StateMachine.start_question)
async def start(message: types.Message, state: FSMContext):
    ...
    await config.StateMachine.question.set()


# Переключение между вопросами 
@dp.message_handler(state=config.StateMachine.question)
async def show_question(message: types.Message, state: FSMContext):
    ...
    if message.text.lower().strip() == "назад":
        ...
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True, selective=True)
        markup.add('Нет', 'Назад', 'Да')
        await config.StateMachine.question.set() # Показываем следующий вопрос
        await message.answer("\n".join((
                "Одинаковые навыки?",
                f"1️⃣ \t{couple_skills.demand_name.capitalize()}",
                f"1️⃣ \t{couple_skills.dup_demand_name.capitalize()}",
